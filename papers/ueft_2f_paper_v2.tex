...
Unified Entanglement Field Theory (UEFT) proposes that information, expressed as entanglement patterns of fundamental fields, underlies both spacetime geometry and material phenomena \cite{werner2024}. The theory posits a single framework applicable from lattice gauge theories ($\text{GL}(3,\mathbb{C})$ link variables) to cosmological perturbations and even cellular coherence in biology.
...
Beyond parameter fitting, UEFT‑2F offers a conceptual shift: \emph{coherence} becomes a dynamical quantity that governs energy exchange between dark components. This viewpoint resonates with information‑theoretic approaches to gravity \cite{jacobson1995} and provides a bridge to biological systems, where coherence–decoherence dynamics regulate cellular function \cite{werner2024b}. The same mathematical structure—a flow equation driving a system toward an attractor—appears in cosmological, lattice‑gauge, and biomedical contexts, suggesting a deep universality.
...
Unified Entanglement Field Theory (UEFT) posits that the fundamental degrees of freedom are complex‑valued link variables $V_\ell \in \text{GL}(3,\mathbb{C})$ living on the edges of a lattice. The action consists of a mass term $\mu^2|V|^2$, a self‑interaction $g|V|^4$, and a holonomy term $\kappa\,\text{Re}\,\text{tr}(V_\triangle)$ that enforces coherence across lattice plaquettes. In the continuum limit, the system exhibits a phase transition at a critical coupling $\kappa_{\text{crit}} \approx 0.89$, separating a disordered (decoherent) phase from an ordered (coherent) one.

To connect lattice UEFT to cosmology, we coarse‑grain over many lattice sites and identify the local order parameter $A_{\text{eq}}(\kappa)$ with the cosmological coherence fraction $f(a)$. The lattice coupling $\kappa$ maps to the dimensionless cosmological coupling $\xi(a) \equiv Q/(H\rho_d)$. The phase transition in the lattice corresponds to the cosmological transition from decoherence‑dominated early times ($f \ll f^*$) to coherence‑dominated late times ($f \to f^*$).
...
\subsection{Connection to Lattice UEFT}
Gitter-UEFT simulations \cite{werner2024} study GL(3,$\mathbb{C}$) link variables on periodic lattices ($L=36,64$). The order parameter $A_{\text{eq}}(\kappa)$ shows a phase transition at $\kappa_{\text{crit}} \approx 0.89$. We identify:
\begin{align}
\kappa &\leftrightarrow \xi(a) \quad \text{(coupling strength)}, \\
A_{\text{eq}} &\leftrightarrow f(a) \quad \text{(coherence parameter)}, \\
L &\leftrightarrow 1/H(a) \quad \text{(system size)}.
\end{align}
Hysteresis in lattice simulations (warm\_forward vs warm\_backward) suggests possible multiple cosmological attractors.

\section{UEFT‑2F Lite: A Minimal Scale‑Dependent Model}\label{sec:lite}

\subsection{Motivation for a Minimal Model}
The full UEFT‑2F model has seven free parameters ($H_0$, $\Omega_m$, $\alpha_0$, $\gamma_0$, $\mu_0$, $a_{\text{trans}}$, $f^*$), which can be reduced by leveraging insights from lattice simulations. The lattice‑measured coherence length $\xi_{\text{coh}} \approx 2.7$ lattice units provides a characteristic scale $k^* \approx 0.02\;h/\text{Mpc}$ that fixes the onset of scale‑dependent growth suppression. Moreover, the phase‑transition analysis suggests a natural attractor value $f^* = 0.70$ and a transition redshift $z_{\text{trans}} \approx 1.9$ ($a_{\text{trans}} = 0.35$). The coupling strength $\alpha_0$ can be set to $0.8$ (the value that reproduces the observed dark‑energy fraction today), and the gravitational modulation $\mu_0$ can be set to zero without worsening the fit to current data.
...
\item $a_{\text{trans}} = 0.35$ ($z \approx 1.9$): The lattice phase transition occurs at a specific coupling strength; mapped to cosmology, this corresponds to a transition redshift around $z \sim 2$, consistent with the onset of dark‑energy domination.
...
Figure~\ref{fig:lattice} illustrates the lattice‑cosmology mapping, comparing the phase transition in lattice UEFT simulations with the cosmological evolution of the coupling $\xi(a)$.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{ueft2f_lattice_cosmology_mapping.pdf}
\caption{Lattice‑cosmology mapping: left panel shows the phase transition in lattice UEFT simulations ($\kappa_{\text{crit}}=0.889$); right panel shows the cosmological evolution of the coupling $\xi(a)$ ($\xi_{\text{crit}} \approx 0.9$). The quantitative consistency ($\Delta=0.011$) supports the identification $\kappa \leftrightarrow \xi$.}
\label{fig:lattice}
\end{figure}
...
The UEFT framework elevates \emph{coherence} from a mere phenomenological descriptor to a fundamental dynamical quantity. In cosmology, it governs the energy exchange between dark components; in lattice gauge theory, it determines the phase of the system; in cellular biology, it regulates metabolic and epigenetic coordination. This tripartite appearance of the same mathematical structure—a flow equation driving a system toward an attractor—suggests that coherence might be a universal organizational principle across scales.
...
\subsection{Bridging Scales: From Lattice to Cosmos to Cell}
...
\item \textbf{Biomedical:} Cellular coherence quantified by the F‑Lab‑Index, linking metabolism, redox state, mitochondrial potential, and epigenetic marks \cite{werner2024b}.
\end{itemize}
The same flow equation $\dot{f} = \alpha (f - f^*)$ appears in all three domains, with parameters adapted to the relevant energy and time scales. This is not a mere analogy but a consequence of the universal role that information entanglement plays in complex systems.
...
UEFT offers a concrete mathematical language—coherence fraction $f$, coupling $\xi$, friction $\gamma$—that can be shared by cosmologists, condensed‑matter physicists, and systems biologists. This shared vocabulary could facilitate unexpected cross‑fertilization: for instance, techniques from lattice gauge theory might help analyze cellular networks, while biomedical time‑series data could provide new insights into non‑equilibrium attractor dynamics.

\subsection{Ethical and Existential Reflections}
A theory that bridges cosmology and medicine inevitably touches on existential questions. If cosmic coherence influences structure formation, and cellular coherence influences health, could there be a deeper connection between the ``health'' of the Universe and the well‑being of its inhabitants? While speculative, such parallels remind us that science is not just about equations but about understanding our place in a coherent, evolving whole.
